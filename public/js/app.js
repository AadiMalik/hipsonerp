$(document).ready(function () {
    ...
    $(document).on('click', '#close-register', function (e) {
        e.preventDefault(); // form ko default submit hone se roke
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.redirect_url) {
                    window.location.href = response.redirect_url; // JS se redirect
                }
            },
            error: function (xhr) {
    ...
    $(document).on('click', '#open-register', function (e) {
        ...
        let url = form.attr('action');      // action URL (/cash-register)
        let formData = form.serialize();    // serialize inputs
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.redirect_url) {
                    // redirect to SellPosController create
                    window.location.href = response.redirect_url;
                } else {
                    toastr.error("Redirect URL not found.");
                }
            },
            error: function (xhr) {
    ...
    $(document).on('click', '#close-register', function (e) {
        ...
        let form = $(this).closest('form'); // get the form inside modal
        let url = form.attr('action'); // get action URL
        let formData = form.serialize(); // serialize all form inputs
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.success) {
                    toastr.success(response.msg); // show success toast
                    $('#close-register-modal').modal('hide'); // close modal
                    // reload page or refresh table if needed
                    location.reload();
                } else {
    ...
    $(document).on('click', '.tw-dw-btn-primary', function (e) {
        e.preventDefault();
        var container = $($(this).data('container'));
        var href = $(this).data('href');
    ...
    $(document).on('click', '#update_sale_commission', function (e) {
        e.preventDefault();

        var form = $('#sale_commission_agent_form');
        var data = form.serialize();

        $.ajax({
            method: 'POST', // always POST (Laravel will use hidden _method=PUT)
            url: form.attr('action'),
            data: data,
            dataType: 'json',
            success: function (result) {
